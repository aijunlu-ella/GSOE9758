...
order.pop(0)
msgbox(msg = 'Got the orders',title = 'Initialized', ok_button='OK')
			...
			with open(filename) as csvfile:
				...
				for name,lat,lng,start,end,load,priority in reader:
					if name == order[0][1]:
						...
						if retrive_position_time in range (int(time.time())-72060,int(time.time())):
							...
							if not lat_diff:
								if not lat_diff:
									if not load_diff:
										msgbox('location matched and weight matched,write into success.csv')
										order.pop(0)
										finish_order = 1
									else:
										msgbox('check item')
